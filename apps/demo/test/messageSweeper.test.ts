...
async function makeMerchantAndCard(): Promise<Fixture> {
  const merchant = await prisma.merchant.create({
    data: { subStatus: 'trialing', trialEndsAt: new Date(Date.now() + 7 * 24 * 60 * 60 * 1000), email: `messagesweeper-test-${randomHex(8)}@example.test`, name: 'Message Sweeper Test Merchant' },
  });
